...
from booking_project.database.models import Hotel
from booking_project.database.schema import HotelInputSchema, HotelOutSchema
...
async def update_hotel(hotel_id: int, hotel: HotelInputSchema,
    ...
    db.commit()
    db.refresh(hotel_db)
    return {'massage': 'Отель озгорулду'}
...
async def delete_hotel(hotel_id: int, db:  Session = Depends(get_db)):
    ...
    db.delete(hotel_db)
    db.commit()
    return {'massage': 'Отель удалить болду'}
