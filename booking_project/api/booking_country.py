...
from booking_project.database.models import Country
from booking_project.database.schema import CountryInputSchema, CountryOutSchema
...
async def update_country(country_id: int, country: CountryInputSchema,
    ...
    country_db = db.query(Country).filter(Country.id == country_id).first()
    if not country_db:
        raise HTTPException(detail='Мындай мамлекет жок', status_code=400)
    ...
    db.commit()
    db.refresh(country_db)
    return {'massage': 'Мамлекет озгорулду'}
...
async def delete_country(country_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    return {'massage': 'Мамлекет удалить болду'}
