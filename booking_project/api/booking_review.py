...
from booking_project.database.models import Review
from booking_project.database.schema import ReviewInputSchema, ReviewOutSchema
...
async def detail_review(review_id: int, db: Session = Depends(get_db)):
    review_db = db.query(ReviewInputSchema).filter(Review.id == review_id).first()
    if not review_db:
