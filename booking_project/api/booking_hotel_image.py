...
from booking_project.database.models import HotelImage
from booking_project.database.schema import HotelImageInputSchema, HotelImageOutSchema
from booking_project.database.db import SessionLocal
from typing import List
from sqlalchemy.orm import Session

hotel_image_router = APIRouter(prefix='/hotel_images', tags=['HotelImages'])
...
async def update_hotel_image(hotel_image_id: int, hotel_image: HotelImageInputSchema,
    ...
    db.commit()
    db.refresh(hotel_image_db)
    return {'massage': 'Отельдин суроту озгорулду'}
...
async def delete_hotel_image(hotel_image_id: int, db:  Session = Depends(get_db)):
    ...
    db.commit()
    return {'massage': 'Отельдин суроту удалить болду'}
